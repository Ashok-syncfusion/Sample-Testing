    ...
    onPropertyChanged(newProp, oldProp) {
        for (const prop of Object.keys(newProp)) {
            switch (prop) {
                ...
                case 'mask':
                    {
                        const strippedValue$$1 = this.value;
    ...
    chunkUploadInProgress(e, metaData, custom) {
        ...
        if (!isNullOrUndefined(liElement)) {
            ...
            if (!isNaN(Math.round((e.loaded / e.total) * 100)) && isNullOrUndefined(this.template) && metaData.file.statusCode !== '4') {
                const loadedSize = (metaData.chunkIndex * this.asyncSettings.chunkSize);
                let currentLoaded = e.loaded;
                if (currentLoaded > this.asyncSettings.chunkSize) {
                    currentLoaded = this.asyncSettings.chunkSize;
                }
                const value = Math.min((((loadedSize + currentLoaded) / metaData.file.size) * 100), 100);
