...
window.sfBlazor.Maps = function () {
  ...
  var SfMaps = function () {
    ...
    SfMaps.prototype.mouseUp = function (event) {
      ...
      if (id.indexOf('_LayerIndex_') > -1) {
        ...
        if (touchCheck && id.indexOf('MarkerIndex') > -1 && !(id.indexOf('cluster') > -1)) {
          ...
          this.dotNetRef.invokeMethodAsync('TriggerMouseLeave', layerIndex_1, markerIndex, dataIndex, id, clientValue.x, clientValue.y, touchCheck);
        }
      }
    ...
    SfMaps.prototype.mouseMove = function (event) {
      ...
      if (this.previousMarkerId.indexOf('_MarkerIndex_') > -1 && this.previousMarkerId != id) {
        ...
        this.dotNetRef.invokeMethodAsync('TriggerMouseLeave', layerIndex, markerIndex, dataIndex, id, clientValue.x, clientValue.y, touchCheck);
      }
    ...
    SfMaps.prototype.handleHighlight = function (element, type, settings) {
      ...
      if (type !== 'Arrow') {
        ...
        element.setAttribute('opacity', settings.opacity);
      }
    };
    ...
    SfMaps.prototype.highlightStyle = function (style, className, setting, elementId) {
      if (elementId.indexOf('NavigationIndex') > -1) {
        ...
        if (this.navigation[layerIndex][lineIndex].Angle === 0) {
          ...
          style.innerHTML = '.' + className + ' {fill: none;opacity:' + setting['Opacity'] + ';stroke:' + setting['Fill'] + ';stroke-width:' + setting['BorderWidth'];
        }
      } else {
        style.innerHTML = '.' + className + ' {fill:' + setting['Fill'] + ';opacity:' + setting['Opacity'] + ';stroke:' + setting['BorderColor'] + ';stroke-width:' + setting['BorderWidth'];
      }
    ...
    SfMaps.prototype.markerTranslate = function (element, factor, x, y, scale, template) {
      ...
      if (latitude && longitude) {
        if (this.isTileMap) {
          ...
          if (element.id.indexOf('_cluster_') !== -1 && element.id.indexOf('_datalabel_') === -1) {
            var clusterBounds = element.getBoundingClientRect();
            point['x'] = point['x'] + clusterBounds.width / 2;
            point['y'] = point['y'] + clusterBounds.height / 2;
          }

          if (element.id.indexOf('_cluster_') !== -1 && element.id.indexOf('_datalabel_') !== -1) {
            var clusterElement = document.getElementById(element.id.split('_datalabel_')[0]);
            var clusterBounds = clusterElement.getBoundingClientRect();
            point['x'] = point['x'] + clusterBounds.width / 2;
            point['y'] = point['y'] + clusterBounds.height / 2;
          }
    ...
    SfMaps.prototype.itemHighlightHandle = function (fill, opacity, layerIndex, settingIndex, type) {
      ...
      if (type === 'Navigation') {
        ...
        if (this.targetElementPath.getAttribute('marker-start') !== '' || this.targetElementPath.getAttribute('marker-end') !== '') {
          ...
          this.handleHighlight(this.targetElementPath, 'Arrow', {
    ...
    SfMaps.prototype.click = function (event) {
      ...
      if (id.indexOf('_LayerIndex_') > -1 && id.indexOf('_MarkerIndex_') > -1) {
        ...
        if (this.markerClusterCollection[layerIndex].Shape === 'Balloon') {
          if (element.firstElementChild.innerHTML.length > 0) {
  ...
  var Maps = {
    ...
    elementAnimate: function elementAnimate(element, delay, duration, x, y, ele, radius, dotNetRef, type, width, markerHeight) {
      var _this = this;
